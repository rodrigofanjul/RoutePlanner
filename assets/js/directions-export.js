...
function createTomTomItineraryItn(gdir, addr, labels) {
  ...
  if (labels[0] != null)
    ...
    label = addr[0];
  var itnStr = getWGS84Degrees(startLatLng.lng) + "|" + getWGS84Degrees(startLatLng.lat) + "|" + label.trim() + "|4|\n";
  for (var i = 0; i < gdir.legs.length; i++) {
    ...
    if (labels[offset] != null)
      ...
      label = addr[offset];
    itnStr += getWGS84Degrees(latLng.lng) + "|" + getWGS84Degrees(latLng.lat) + "|" + label.trim() + "|2|\n";
  }
  return itnStr;
}
...
function getDateStr() {
  ...
  var month = zeroPadded(dateObj.getUTCMonth() + 1, 2); //months from 1-12
  var day = zeroPadded(dateObj.getUTCDate(), 2);
...
function createGarminGpx(gdir, addr, labels) {
  ...
  gpxStr += "<rte><name>OptiMap " + getDateStr() + "</name>";
